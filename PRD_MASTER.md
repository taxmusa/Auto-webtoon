...
**프리셋 스타일 (인물/배경 분리)**:
- 인물 스타일: 기본 웹툰, 흑백 펜화, 소년만화, 플랫 벡터, SD/치비, 레트로 애니, 픽셀 아트, 동글 라인, 파스텔 플랫, 볼드 카툰, 연필 스케치, 모노라인, 수채화, 네온 팝, 이모지, 90s 레트로, 컷아웃, 지브리, 로맨스, 비즈니스 + 단순 선화 10종 + 커스텀 업로드
- 배경 스타일: 애니메이션 배경, 실사풍 배경, 단순 배경 + 커스텀 업로드
- UI: 인물/배경 탭 분리, 4열 카드 그리드, 페이지네이션 (12개/페이지), 프리뷰 이미지
...
인스타 캐러셀 최대 10장. AI 추천 → 사용자 조정 가능.

**발행 흐름**:
- 편별 독립 편집 (1편 수정해도 2편에 영향 없음)
- 편별 개별 발행 (1편 즉시 + 2편 24시간 후 예약 기본)
- 시리즈 간 캐릭터/스타일 자동 동기화
- 2편 캡션에 "1편 보러가기 URL" + "지난 이야기 요약" 자동 삽입

> 상세: WORKFLOW.md §2.11 참조

### 4.5 썸네일(커버 이미지)

캐러셀 첫 번째 장으로 사용되는 전용 커버 이미지. ON/OFF 가능 (기본 ON).

**생성 방법**: AI 자동 생성(기본) / 본편 이미지에서 선택 / 직접 업로드
**텍스트 오버레이**: Pillow로 제목 큰 글씨 오버레이 (위치: 상단/중앙/하단, 색상/크기 조절)
**캐러셀 관계**: 썸네일 ON → 첫 장(1) + 본편(최대 9) = 10장 제한

> 상세: WORKFLOW.md §2.8.5 참조

### 4.6 "다음편에 계속" 시스템

시리즈 마지막 씬에 "다음편에 계속" 오버레이 자동 적용. ON/OFF 가능 (기본 ON).

**디자인 옵션**: 하단 그라데이션(기본) / 우측 하단 배지 / 전체 오버레이
**적용 조건**: 시리즈 분할됨 + 마지막 편이 아닌 경우만

> 상세: WORKFLOW.md §2.8 참조
...
├── STYLE_CONSISTENCY_POLICY.md    ← 스타일 일관성 정책 (GUARD 블록)
├── docs/STYLE_SYSTEM.md           ← 스타일 관리 시스템
└── docs/IMAGE_EDIT_STAGE.md       ← 이미지 편집 단계 상세
```
...
| 2026-02-11 | 1.4.0 | 썸네일 시스템(§4.5), "다음편에 계속"(§4.6), 시리즈 발행 흐름(§4.4), 프리셋 스타일 인물/배경 분리, GUARD 블록 일관성 정책, UI 리팩토링, 문서 구조 보강 |
