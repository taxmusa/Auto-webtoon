...
[샘플 미리보기] ← 1장 Low품질 확인 (비용 절약)
  → 만족 시 진행 / 불만족 시 프롬프트 수정 또는 스타일 변경
      ↓
[이미지 생성]
      ↓
...
- **발행물에서 추출**: 기존 인스타툰 업로드 → Vision AI가 인물/배경 스타일 자동 분석
- **직접 업로드**: 참고 이미지를 올려 인물 or 배경 스타일로 저장
- **조합 선택**: 저장된 인물 스타일 + 배경 스타일을 조합하여 적용
- **샘플 미리보기**: 전체 생성 전 1장 Low 품질 샘플로 확인 (비용 절약)
...
| 2026-02-09 | 1.4.0 | 스타일 관리 시스템 추가 (인물/배경 스타일 분리 저장, 발행물 추출, 샘플 미리보기, 수동 프롬프트 편집), 워크플로우에 스타일 선택+샘플 단계 삽입 |
