...
**구성**:
1. **전체 톤 조절**: 자연어 명령으로 색감/채도 등 일괄 변경 (Pillow 무료 / AI 유료 구분)
2. **씬별 확정**: 각 씬의 이미지를 확정해야 다음 단계 진행 가능
3. **편집 기능**: 재생성(프롬프트 수정), 사용자 이미지 업로드, 되돌리기

**이유**:
- 텍스트 오버레이 후 이미지가 바뀌면 말풍선 위치를 다시 잡아야 하는 비효율 제거
- 원본 이미지 상태에서 톤앤매너를 확정하는 것이 품질 관리에 유리
